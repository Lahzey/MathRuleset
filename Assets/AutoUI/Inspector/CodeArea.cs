...
public class CodeArea {
	...
	private static string HandleKeyDownEvent(string input, CodeAreaState state, out bool canShowAutoComplete) {
		...
		// for keystrokes that should not result in a typed key (like ctrl+c) we only get the KeyCode event, so I suppose that is the purpose of this wierd behaviour
		...
		switch (Event.current.keyCode) {
			...
			case KeyCode.C:
			case KeyCode.A:
				if (Event.current.control) input = HandleCharInput(input, state, Event.current.keyCode.ToString()[0]); // Event.current.keyCode.ToString()[0] extracts the first character of the KeyCode enum name
				break;
			case KeyCode.None:
				char c = Event.current.character;
				input = HandleCharInput(input, state, c);
	...
	private static string HandleCharInput(string input, CodeAreaState state, char c) {
		if (c < 32 || c > 126) return input; // any visible ascii character is in this range
